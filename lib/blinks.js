    ...
    function simulation(scope) {
        ...
        function onMouseDownEvent() {
            ...
            longpressTimer = setTimeout( ()=>{
                ...
                if($._currDragging){
                    $._buttonLongPressedFn($._getBlockIndexOnMouse())
                }
                mouseOnClick = false
                clicks = 0
            }, $.LONGPRESSTIMEOUT)
            ...
            if ($._MatterMouseConstraint.body) {
                ...
                $._buttonDownFn($._getBlockIndexOnMouse())
                return
            }
        ...
        function onMouseUpEvent() {
            ...
            let blockIndex = $._getBlockIndexOnMouse()
            if(blockIndex >= 0){
        ...
        $._createGroups = function (bks, conns) {
            ...
            conns.map(function (conn) {
                ...
                let bi0 = result.reduce(function (prev, curr) { return (curr.blocks.includes(b0.id)) ? result.indexOf(curr) : prev; }, -1) // $._getGroupIndexByBlockID(b0.id)
                let bi1 = result.reduce(function (prev, curr) { return (curr.blocks.includes(b1.id)) ? result.indexOf(curr) : prev; }, -1) // $._getGroupIndexByBlockID(b1.id)
        ...
        $._getBlockIndexOnMouse = function () {
            let result = -1
